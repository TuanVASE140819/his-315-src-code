    ...
    <ul className='flex flex-col gap-2 mt-2 border rounded-md p-2 h-[79.5vh] overflow-auto bg-[#fdfdfd]'>
      {list?.map((item, index) => (
        <li
          ...
          className={`${itemSelected?.id === item?.id ? 'bg-sky-50' : 'bg-white'} hover:bg-sky-100 transition-all duration-300 border rounded-md p-2 flex gap-2 cursor-pointer`}
        >
          <Avatar
            shape='square'
            size={30}
          ...
          <div className='w-full flex justify-start items-center gap-2'>
            ...
            <Checkbox className='w-4 h-4 ml-auto' checked={item?.isActive} />
            <EditOutlined
              onClick={onClickEdit}
